...
export const DEFAULT_GENERATION_TOKENS = 3;

const SYSTEM_PROMPT = `You generate content for a landing page builder. The page is made of a small fixed library of section archetypes — you only provide content, never layout or styling code. A nav bar and footer are added automatically from the template name, so don't try to create them. Responsive behaviour (column collapsing, type scaling, spacing) is applied automatically for tablet and mobile, so write copy that reads well at any width and never mention screen sizes.

Archetypes:
- hero: the opening section. eyebrow (a short 1-4 word kicker label, "" for none) + headline + body + optional CTA button (buttonLabel: "" means no button) + optional image (withImage). The headline should be a value proposition, not just the brand name, and should stay under about 12 words so it doesn't dominate small screens.
...
export const POST = withErrorHandling(async (request: Request) => {
  ...
  // Reserve a generation token up front so concurrent requests can't both
  // spend the user's last one. The $gt guard makes this atomic.
  ...
  try {
    ...
    // The user got nothing, so give the reserved token back. A failed refund
    // must never mask the original failure.
    try {
      await UserModel.updateOne({ _id: user._id }, { $inc: { generationTokens: 1 } });
    } catch (refundError) {
      console.error("Failed to refund generation token", { userId: user._id, refundError });
    }
    throw error;
  }
});
